...
void upwind(const int Nx, const int Ny, double a_x, double a_y, const double CFL) {
  ...
  data_export("fichier0", Nx, Ny, x, y, (const double **)u0, 0);
  for (int t = 0; t < N; t++) {
    for (int fi = 0; fi < Nx + 1; fi++) {
      for (int j = 0; j < Ny + 2; j++) {
    ...
    //sprintf(filename, "monfichier%i", t);
    //std::cout << filename << std::endl;
    //data_export(filename, Nx, Ny, x, y, (const double **)u0, t);
  ...
  data_export("monfichier", Nx, Ny, x, y, (const double **)u0, 1);
